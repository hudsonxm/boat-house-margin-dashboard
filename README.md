...
The live data is three Google Sheets tabs: an ingredient database (purchase sizes and costs), a recipe sheet (ingredient amounts per menu item), and a menu list (selling prices). The sheet currently covers 182 ingredients across 427 recipe rows and 65 menu items. On load, the app:
...
- **Unit conversions live in one place.** Only two exist: `lbs -> oz` (x16) and a business-specific `tea bags -> fl oz` yield calculation; everything else is same-unit. Both are commented branches in one function.
...
The Google Sheet is read through a service account. Copy `.streamlit/secrets.toml.example` to `.streamlit/secrets.toml` and fill in a service-account key JSON under `[gcp_service_account]`; the target sheet must be shared with that account as a viewer, but without it the app still starts and explains what's missing. The live app is private, as it reads a real business's ingredient costs and vendor data.

## What's next

- **Plymouth's exclusive menu.** The second location sells sandwiches, salads, and coffees that aren't in the sheet yet. The `Available At` column already exists to mark which items each location carries; the dashboard needs to filter on it.
- **In-app editing.** Right now a price change means opening the Google Sheet. Editing ingredients and recipes directly in the dashboard would remove that step.
